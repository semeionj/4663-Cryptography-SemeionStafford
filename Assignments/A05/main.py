...
if __name__ == '__main__':
    ...
    #Cipher
    encrypted_text = "vptnvffuntshtarptymjwzirappljmhhqvsubwlzzygvtyitarptyiougxiuydtgzhhvvmumshwkzgstfmekvmpkswdgbilvjljmglmjfqwioiivknulvvfemioiemojtywdsajtwmtcgluysdsumfbieugmvalvxkjduetukatymvkqzhvqvgvptytjwwldyeevquhlulwpkt"
    ...
    for key_length in range(2, 17):
        seq_array.append(encrypted_text[:key_length])
        seq_array.append(encrypted_text[::key_length])
    ...
    for i in range(17):
        ...
        for letter in ALPHABET:
            fi = (seq_array[i].count(letter))
            #print(letter, fi)
            num = fi * (fi - 1)
            index_c += num / den
        print(seq_array[i], index_c)
        i += 1  
